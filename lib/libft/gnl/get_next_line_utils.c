...
/*   Updated: 2025/09/10 22:06:45 by ldurmish         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_clear_list(t_list_gnl **list, t_list_gnl *clean_node, char *buf,
	int del_one)
{
	t_list_gnl	*tmp;
	...
	{
		tmp = *list;
		return (*list = (*list)->next, free(tmp->data), free(tmp), NULL);
	}
	while (*list)
	{
		tmp = (*list)->next;
